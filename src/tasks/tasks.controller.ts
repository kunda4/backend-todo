...
export class TasksController {
    ...
    @ApiResponse({ status: 200, description: 'Successful operation', type: [CreateTaskDto] })
    displayAllTasks(){
        return this.taskService.findAllTasks()
    }

    @Get('/:id')
    @ApiOperation({ summary: 'Get a task by ID', description: 'Get details of a task by its ID' })
    @ApiResponse({ status: 200, description: 'Successful operation', type: CreateTaskDto })
    ...
    @Delete('/:id')
    @ApiOperation({ summary: 'Delete a task by ID', description: 'Delete a task by its ID' })
